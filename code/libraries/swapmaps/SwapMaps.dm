...
swapmap
	...
	proc/Save()
		if(id==src) return 0
		var/savefile/S=mode?(new):new("./data/saves/map_[id].sav")
		S << src
		while(locked) sleep(1)
		if(mode)
			fdel("./data/saves/map_[id].txt")
			S.ExportText("/","./data/saves/map_[id].txt")
		return 1
...
proc/SwapMaps_Load(id)
	...
	if(!M)
		var/savefile/S
		var/text=0
		if(swapmaps_mode==SWAPMAPS_TEXT && fexists("./data/saves/map_[id].txt"))
			text=1
		else if(fexists("./data/saves/map_[id].sav"))
			S=new("./data/saves/map_[id].sav")
		else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("./data/saves/map_[id].txt"))
			text=1
		else return	// no file found
		if(text)
			S=new
			S.ImportText("/",file("./data/saves/map_[id].txt"))
		S >> M
...
proc/SwapMaps_CreateFromTemplate(template_id)
	...
	var/savefile/S
	var/text=0
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("./data/saves/map_[template_id].txt"))
		text=1
	else if(fexists("./data/saves/map_[template_id].sav"))
		S=new("./data/saves/map_[template_id].sav")
	else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("./data/saves/map_[template_id].txt"))
		text=1
	else
...
proc/SwapMaps_LoadChunk(chunk_id,turf/locorner)
	...
	var/savefile/S
	var/text=0
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("./data/saves/map_[chunk_id].txt"))
		text=1
	else if(fexists("./data/saves/map_[chunk_id].sav"))
		S=new("./data/saves/map_[chunk_id].sav")
	else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("./data/saves/map_[chunk_id].txt"))
		text=1
	else
	...
	if(text)
		S=new
		S.ImportText("/",file("./data/saves/map_[chunk_id].txt"))
	/*
		This hacky workaround is needed because S >> M will create a brand new
		M to fill with data. There's no way to control the Read() process
		properly otherwise. The //.0 path should always match the map, however.
