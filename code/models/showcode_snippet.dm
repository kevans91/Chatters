/showcode_snippet
	...
	proc/Send(code = 0)
		if(!target)
			...
			MsgMan.RouteMsg(ChatMan.Get(owner), ChatMan.Get(target), "[owner] has sent a private [code ? "code" : "text"] snippet.  <a href='byond://?src=\ref[ChatMan]&target=\ref[ChatMan.Get(owner)]&action=show[code ? "code" : "text"]&index=[id]'>Show [code ? "Code" : "Text"]</a>", 1)
